...
#made the image between normalised the image
...
#that is total number of filter is 1 in the layer 
#conv_filter.shape[0] is the number of filters which is '1' filter in qn 1
#conv_filter.shape[1] conv_filter.shape[2] filter dimensions 
#conv_filter.shape[2] is the depth of the kernel=input depth
#random weights for convolution filter kernel

conv_filter=np.random.normal(1e-4,1,(1,5,5,img.shape[2]))

print img.shape[2]
stride=(1,1)
padding='same'
Activation_function='relu'
feature_maps1=conv(img,conv_filter,padding,stride,Activation_function)
...
# print('FILTER KERNEL:'+str(conv_filter.shape))
# print(conv_filter)

plt.imshow(feature_maps1[:,:,0],cmap="gray")
plt.show()
# plt.imshow(apply_activationCONV('relu',feature_maps[:,:,0]-feature_maps1[:,:,0]),cmap="gray")
# plt.show()